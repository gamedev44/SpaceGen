...
class WORLDSCAPENOISE_API UIceWorld : public UNoiseParameter
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "General")
		float CraterSurfaceApperanceInfluence = 4;
...
public:
	...
	static double GetCrater(CustomNoise NoiseClass, DVector position, UIceWorld* NoiseParam, int SizeGeneration);
	static double GetCraterFractal(CustomNoise NoiseClass, DVector position, UIceWorld* NoiseParam, int octave, double lacunarity, double persistence, bool Min = false);
};

UCLASS()
class WORLDSCAPENOISE_API UIceWorldNoise : public UWorldScapeNoiseClass
{
