...
double Hill_Biome4::GetNoise(CustomNoise NoiseClass, DVector position, int octave, float lacunarity, float persistence) {
	return NoiseClass.Fractal(position, octave, lacunarity, persistence);
}


//Crater Noise Generation for the moon
double CraterGeneration2::CavityShape(double distance)
{
	return distance * distance;

}
double CraterGeneration2::RimShape(double distance, double rimSteepness, int power)
{
	distance = distance - 1;
	if (power == 1)
...
double CraterGeneration2::CraterShape(double distance, double rimSteepness, double Height, double smoothness, int rimPower)
{
...
double CraterGeneration2::GetCrater(CustomNoise NoiseClass, DVector position, UIceWorld* NoiseParam, int SizeGeneration)
{
...
double CraterGeneration2::GetCraterFractal(CustomNoise NoiseClass, DVector position, UIceWorld* NoiseParam, int octave, double lacunarity, double persistence, bool Min)
{
...
FNoiseData UIceWorldNoise::GetHeight(CustomNoise NoiseClass, DVector position, DVector PlanetPosition, double NoiseScale, double NoiseIntensity, double PlanetScale, bool FlatWorld, double lattitude, DVector& NoisePosition, FNoiseData ActualData, bool UsePlanetary, double& LandMassNoise)
{
	...
	*/
	...
	if (FlatWorld) {
		...
		Data.Humidity = NoiseClass.Fractal(WarpedPosition2 * NoiseParameters->HumidityScale, 4);
	...
	double Ridge = ((pow(NoiseClass.FractalRidge(RiverWarpedPosition1 * 1.5f * NoiseParameters->RidgeScale, 1), NoiseParameters->RidgeShaprness) * NoiseParameters->RidgeHeight) * (NoiseClass.Fractal(WarpedPosition * 1.5f * NoiseParameters->RidgBump, 4) + NoiseParameters->RidgOffset));

	double Hillmask = NoiseMathUtils::Clamp01(NoiseClass.Fractal(WarpedPosition * 1.5f * NoiseParameters->surfaceFeature, 2) * 1.2);
	double SurfaceDeformation = (NoiseClass.Fractal(WarpedPosition * WarpedPosition2 * 1.5f * NoiseParameters->SurfaceDeformationScale, NoiseParameters->SurfaceDeformationOcative, NoiseParameters->SurfaceDeformationLacunarity, NoiseParameters->SurfaceDeformationPersistence) * NoiseParameters->SurfaceDeformationHeight);
	double Deformationmask = NoiseMathUtils::Clamp01(pow((NoiseClass.Fractal(WarpedPosition * 1.5f * NoiseParameters->surfaceFeature, 2) * 1.5), 4));

	const double HillNoise = Hill_Biome4::GetNoise(NoiseClass, WarpedPosition * NoiseParameters->HillScale, NoiseParameters->HillOctave, NoiseParameters->HillLacunarity, NoiseParameters->HillPersistence) * NoiseParameters->HillHeight;
	Data.HeightNormalize = CraterGeneration2::GetCraterFractal(NoiseClass, NoisePosition * 0.002f * NoiseParameters->CraterFrequency, NoiseParameters, NoiseParameters->CraterOctave, NoiseParameters->CraterLacunarity, NoiseParameters->CraterPersistence);
	Data.Humidity *= (((Data.HeightNormalize - NoiseParameters->CraterSurfaceApperanceInfluence) / 5) + (Ridge / NoiseMathUtils::Clamp(NoiseParameters->RidgeHeight * 3, 1, 50000)));
	Data.HeightNormalize *= 0.9f + 0.05f * Hill_Biome4::GetNoise(NoiseClass, NoisePosition * 0.1f, 4, 2, 0.5) + 0.05f * CraterGeneration2::GetCraterFractal(NoiseClass, NoisePosition * 0.5f * NoiseParameters->CraterFrequency, NoiseParameters, NoiseParameters->CraterOctave, NoiseParameters->CraterLacunarity, NoiseParameters->CraterPersistence);;
	Data.HeightNormalize = Data.HeightNormalize + (HillNoise * Hillmask) + (SurfaceDeformation * Deformationmask) - Ridge;
...
FNoiseData UIceWorldNoise::GetNoise(CustomNoise NoiseClass, DVector position, DVector PlanetPosition, double NoiseScale, double NoiseIntensity, double PlanetScale, bool FlatWorld, double lattitude, DVector& NoisePosition, FNoiseData ActualData, bool UsePlanetary)
{
...
FNoiseData UIceWorldNoise::GetOceanNoise(CustomNoise NoiseClass, DVector position, DVector PlanetPosition, double NoiseScale, double NoiseIntensity, double PlanetScale, bool FlatWorld, double lattitude, DVector& NoisePosition, FNoiseData ActualData, bool UsePlanetary)
{
...
UIceWorldNoise::UIceWorldNoise() {
	NoiseParameters = CreateDefaultSubobject<UIceWorld>(TEXT("Noise Parameters"));
}